...
# 수정한 Linear class (w_init 인자 추가로 he초기값 사용 가능)


class Linear(Layer):
    def __init__(self, out_size, nobias=False, dtype=np.float32, in_size=None, w_init='xavier'):
        super().__init__()
    ...
    def _init_W(self):
        ...
        if self.w_init == 'he':
            W_data = np.random.randn(I, O).astype(self.dtype) * np.sqrt(2/I)
        else:
